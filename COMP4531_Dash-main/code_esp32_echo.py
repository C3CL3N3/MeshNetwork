...
#
# LoRa Mesh Echo Node -- XIAO ESP32-S3 + SX1262
# H/R/D protocol (compatible with nRF gateway). Auto-replies ECHO:<msg> to
# every received message source. Used for round-trip latency / hop testing.
# Serial commands same as standard relay node.
...
try:
    ...
    print("Echo Node {}  {} MHz  SF{}  TTL={}".format(
        NODE_ID, MESH_FREQ, mc.network_sf, mc.TTL_DEFAULT))
    logger.init()
...
def _deliver(src, dst, payload):
    ...
    if payload.startswith("ECHO:") or payload.startswith("PARROT:") or src == 0:
        return
    # Stagger by node ID: 150 ms gaps between simultaneous echo nodes
    time.sleep(0.15 * NODE_ID)
    reply = "ECHO:{}".format(payload)
    # Prefer unicast; fall back to broadcast if no route yet established
    if mc.route_next_hop(src) is not None:
        send_data(src, reply)
    else:
        print("  ^ no route to N{}, broadcasting echo".format(src))
        send_data(0, reply)
    print("  ^ ECHO -> N{}: '{}'".format(src, reply))
...
# -- Main ----------------------------------------------------------------------
print("Echo relay running -- Node {}".format(NODE_ID))
