...
class SX1262:
    ...
    def begin(self, freq=912.0, bw=125.0, sf=7, cr=5, power=22,
        ...
        self._tcxo_voltage = tcxoVoltage
        self._use_ldo = useRegulatorLDO
        ...
        if tcxoVoltage > 0:
            ...
            self._cmd(_SET_DIO3_TCXO, v, 0x00, 0x02, 0x80)  # delay ~10 ms
            time.sleep(0.015)

        if debug: print("SX1262 begin: calibrate")
        self._cmd(_CALIBRATE, 0xFF)
        time.sleep(0.050); self._wait_busy(1000)
    ...
    def send(self, data, timeout_ms=5000):
        ...
        Recovers radio to standby on any failure so subsequent operations succeed.
        ...
        tx_ticks = min(round(int(timeout_ms) * 1000 / 15.625), 0xFFFFFF)
        self._tx_mode()
        self._cmd(_SET_TX,
                  (tx_ticks >> 16) & 0xFF,
                  (tx_ticks >> 8) & 0xFF,
        ...
        # Always clean up — even if TX timed out, radio may still be busy.
        # Use try/except so a stuck BUSY doesn't skip the recovery path.
        try:
            self._rx_mode()
            self._clr_irq()
        except RuntimeError:
            # BUSY is stuck high — force radio back to standby via raw SPI
            self._force_standby()
    ...
    def _force_standby(self):
        """Recover radio from a stuck-BUSY state. Tries graceful standby first,
        then falls back to a full reset-pin toggle and re-init if unresponsive."""
        # Attempt 1: longer busy wait — BUSY might just be slow to clear
        try:
            self._wait_busy(500)
        ...
        # Attempt 2: hardware reset + full re-init
        try:
            ...
            time.sleep(0.015)
            self._wait_busy(500)
            # Re-initialise with saved parameters
            self.begin(freq=self._freq, bw=self._bw, sf=self._sf, cr=self._cr,
                       tcxoVoltage=getattr(self, '_tcxo_voltage', 1.8),
                       useRegulatorLDO=getattr(self, '_use_ldo', False))
        except Exception:
            pass
